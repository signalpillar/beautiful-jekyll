...
I've tried almost all the libraries from the list below. *pydantic* is my
favorite at the moment.

[[https://pypi.org/project/pydantic/][pydantic]] another data validation and settings management library that heavily
relies on type hints. The validation errors it
produces can be used directly as part of the Web API error reporting.
...
attributes just using pure type annotations it understands how to map the
fields. Unfortunately the licence doens't allow me to use the library in the
proprietary projects.

[[https://python-typical.org][typical]] library is similar to pydantic. It suggests nice extra types (secret,
URL etc) and claims to be the fastest type validation library. Relies on
dataclasses (3.7).
